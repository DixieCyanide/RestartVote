...
public class VotePositive implements CommandExecutor{
    RestartVote plugin;

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("rvote.vote.positive") && !sender.isOp()) {
            return false;
        }
        ...
        if (!plugin.getVotedPlayerUUID().contains(UUID)) {
            Integer votedPlayerCount = plugin.getVotedPlayerCount() + 1;
            ...
            plugin.setVotedPlayerCount(votedPlayerCount);
